...
const generateChatResponse = async (req, res) => {
  try {
    ...
    const lastMessages = await Conversation.find({ user: userId })
    .sort({ createdAt: -1 })
    .limit(3);

    let previousContext = "";
    lastMessages.forEach(msg => {
      previousContext += `User: ${msg.userMessage}\n`;
    });
    ...
    try {
      ...
      aiResponse = await generateGroqResponse(aiContext, message);
...
function prepareAIContext(message, context, mood) {
  let aiContext = `You are CalmSpace AI Assistant, a supportive mental health companion specifically designed for students. Your role is to provide empathetic, non-judgmental, and helpful support.
...
`;
  ...
  aiContext += `\nUSER'S MESSAGE: "${message}"\n\n
    IMPORTANT:
    - Respond directly to the USER'S MESSAGE (not just mood)
    - If user says "hi" → greet naturally
    - If user shares problem → respond specifically to that problem
    - Avoid repeating same sentences
    - Make response feel personal and conversational

    Provide a natural, human-like response:
  `;
...
function generateFallbackResponse(message, mood) {
  const lowerMessage = message.toLowerCase();
  // 🎯 Basic conversation handling (VERY IMPORTANT)
  if (lowerMessage === 'hi' || lowerMessage === 'hello') {
    return "Hey 😊 I'm really glad you came here. How are you feeling today?";
  }

  if (lowerMessage.includes('how are you')) {
    return "I'm here with you 💙 Tell me about your day — how are you feeling right now?";
  }
  ...
  if (mood && mood.label) {
    switch (mood.label.toLowerCase()) {
      ...
      case 'happy':
        return "That's nice to hear 😊 What made your day good today?";
  ...
  if (lowerMessage.includes('sad') || lowerMessage.includes('hurt') || lowerMessage.includes('cry')) {
  return "I'm really sorry you're feeling this way 💙 Sometimes things hurt more than we expect. Do you want to tell me what happened?";
  }

  if (lowerMessage.includes('scold') || lowerMessage.includes('shout')) {
    return "That must have felt really upsetting 😔 Being shouted at can stay in our mind for a long time. Are you okay right now?";
  }

  if (lowerMessage.includes('motivation')) {
    return "Hey 💪 you're stronger than you think. You’ve already come this far. What’s one small thing you can do right now?";
  }
  ...
  const random = Math.floor(Math.random() * supportiveResponses.length);
  return supportiveResponses[random] + " 💬";
}
