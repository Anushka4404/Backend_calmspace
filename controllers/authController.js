  ...
  try {
    ...
    const htmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
        <div style="text-align: center; margin-bottom: 20px;">
          <h1 style="color: #4073c0;">CalmSpace</h1>
        </div>
        <div style="margin-bottom: 30px;">
          <h2 style="color: #333;">Verify Your Account</h2>
          <p>Thank you for registering with CalmSpace. To complete your registration, please use the following OTP code:</p>
        ...
        <div style="color: #666; font-size: 14px; border-top: 1px solid #e0e0e0; padding-top: 20px;">
          ...
          <p>© ${new Date().getFullYear()} CalmSpace. All rights reserved.</p>
        </div>
      </div>
    `;
    ...
    try {
      ...
      const emailResult = await sendEmail({
        to: email,
        subject: 'CalmSpace Account Verification',
...
exports.resendOTP = async (req, res, next) => {
  try {
    ...
    try {
      await sendEmail({
        to: email,
        subject: 'CalmSpace Account Verification - New OTP',
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    try {
      await sendEmail({
        to: email,
        subject: 'CalmSpace Password Reset',
